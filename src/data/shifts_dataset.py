...
class ShiftsDataset(Dataset):
    ...
    def __init__(self,
        ...
        self._raw_file_names = [os.path.join(
            *f.split(os.sep)[-2:]) for f in self._raw_file_paths]
        self._processed_file_names = [os.path.splitext(
            f)[0] + ".pt" for f in self._raw_file_names]
        self._processed_paths = [os.path.join(
            self.processed_dir, f) for f in self._processed_file_names]
    ...
    def create_process_args(self):
        ...
        for i, raw_path in enumerate(self.raw_paths):
            ...
            args.append([raw_path, self._split, self._scene_tags[i],
                        self._local_radius, self.processed_paths[i]])
...
def extract_ego_vehicle_trajectory(track, t_init=0, ctrack_id=None, sobject_type="ego"):
    ...
    for t, agent in enumerate(track):
        ...
        vehicle = 1
        x, y, z, xdim, ydim, zdim, vx, vy, ax, ay, yaw = get_agent_state(
            agent, object_type="ego")

        trajectory_dict = append_trajectory_dict(
            trajectory_dict, timestamp, track_id, object_type, vehicle, x, y, vx, vy, ax, ay, yaw)
...
def extract_trajectories_from_pb_tracks(pb_tracks, t_init=0, sobject_type="vehicle"):
    ...
    for t, timestep in enumerate(pb_tracks):
        for agent in timestep.tracks:
            ...
            x, y, z, xdim, ydim, zdim, vx, vy, ax, ay, yaw = get_agent_state(
                agent, object_type=sobject_type)

            vehicle = 1
            if sobject_type == "pedestrian":
                vehicle = 0

            agent_trajectories = append_trajectory_dict(
                agent_trajectories, timestamp, track_id, object_type, vehicle, x, y, vx, vy, ax, ay, yaw)
...
def set_target_agents(df, prediction_requests):
    ...
    df.loc[(df["TRACK_ID"].isin(prediction_requests)) & (df["VEHICLE"] == 1),
           "OBJECT_TYPE"] = "AGENT"  # only vehicles are predicted in shifts
...
def get_one_hot_trajectory_tags_mat(prediction_requests):
    ...
    one_hot_tags_mat = torch.zeros(
        (len(prediction_requests), len(get_trajectory_tags())), dtype=torch.bool)
...
def process_shifts(args: list) -> None:
    ...
    # vehicle tracks
    df_past_vehicle_tracks = extract_trajectories_from_pb_tracks(
        pb.past_vehicle_tracks, t_init=0, sobject_type="vehicle")  # [N(t)*T(N), 11]
    df_future_vehicle_tracks = extract_trajectories_from_pb_tracks(
        pb.future_vehicle_tracks, t_init=25, sobject_type="vehicle")  # [N(t)*T(N), 11]

    # pedestrian tracks
    df_past_pedestrian_tracks = extract_trajectories_from_pb_tracks(
        pb.past_pedestrian_tracks, t_init=0, sobject_type="pedestrian")  # [N(t)*T(N), 11]
    df_future_pedestrian_tracks = extract_trajectories_from_pb_tracks(
        pb.future_pedestrian_tracks, t_init=25, sobject_type="pedestrian")  # [N(t)*T(N), 11]

    # combine dataframes
    df = pd.concat([df_past_vehicle_tracks, df_future_vehicle_tracks,
                   df_past_pedestrian_tracks, df_future_pedestrian_tracks], ignore_index=True)
    ...
    df_past_ego_trajectory = extract_ego_vehicle_trajectory(
        pb.past_ego_track, t_init=0, ctrack_id=ego_track_id, sobject_type="ego")  # [T_o, 11]
    df_future_ego_trajectory = extract_ego_vehicle_trajectory(
        pb.future_ego_track, t_init=25, ctrack_id=ego_track_id, sobject_type="ego")  # [T_p, 11]

    # combine dataframes
    df = pd.concat([df, df_past_ego_trajectory,
                   df_future_ego_trajectory], ignore_index=True)
    ...
    # in shifts 25 historical time steps = 5s @ 5 Hz
    historical_timestamps = timestamps[: n_past_ts]
    historical_df = df[df['TIMESTAMP'].isin(historical_timestamps)]
    historical_track_id_vehicle_df = historical_df[[
        "TRACK_ID", "VEHICLE"]].drop_duplicates()
    # track-ids are not unique, might be the same for vehicle and pedestrian
    num_nodes = historical_track_id_vehicle_df.shape[0]
    select_valid_actors = [(item.tolist() in historical_track_id_vehicle_df.values.tolist(
    )) for item in df[["TRACK_ID", "VEHICLE"]].values]
    ...
    av_index = actor_identifications.index(
        [av_df[0]['TRACK_ID'], av_df[0]['VEHICLE']])
    agent_index = [i for i in range(len(actor_identifications)) if actor_identifications[i]
                   [0] in prediction_requests and actor_identifications[i][1] == 1]
    city = df['CITY_NAME'].values[0]

    # make the scene centered at AV
    origin = torch.tensor(
        [av_df[n_past_ts-1]['X'], av_df[n_past_ts-1]['Y']], dtype=torch.float)
    av_heading_vector = origin - \
        torch.tensor([av_df[n_past_ts-2]['X'], av_df[n_past_ts-2]
                     ['Y']], dtype=torch.float)
    ...
    edge_index = torch.LongTensor(
        list(permutations(range(num_nodes), 2))).t().contiguous()
    ...
    for group_id, actor_df in df.groupby(['TRACK_ID', 'VEHICLE']):
        ...
        node_steps = [timestamps.index(timestamp)
                      for timestamp in actor_df['TIMESTAMP']]
        # no padding only when agent is present
        padding_mask[node_idx, node_steps] = False
        # make no predictions for actors that are unseen at the current time step
        if padding_mask[node_idx, 24]:
            padding_mask[node_idx, 25:] = True
        # position (m)
        xy = torch.from_numpy(
            np.stack([actor_df['X'].values, actor_df['Y'].values], axis=-1)).float()
        x[node_idx, node_steps] = torch.matmul(xy - origin, rotate_mat)
        # velocity  (in m/s)
        vxy = torch.from_numpy(
            np.stack([actor_df['VX'].values, actor_df['VY'].values], axis=-1)).float()
        v[node_idx, node_steps] = torch.matmul(vxy, rotate_mat)
        # acceleration (in m/s^2)
        axy = torch.from_numpy(
            np.stack([actor_df['AX'].values, actor_df['AY'].values], axis=-1)).float()
        a[node_idx, node_steps] = torch.matmul(axy, rotate_mat)
        # yaw
        yaw[node_idx, node_steps] = torch.from_numpy(
            actor_df["YAW"].values)[:, None].float()
        # historical time steps
        node_historical_steps = list(
            filter(lambda node_step: node_step < 25, node_steps))
        # is vehicle
        is_vehicle[node_idx] = torch.tensor(
            actor_df['VEHICLE'].iloc[0], dtype=torch.bool)
        ...
        if len(node_historical_steps) > 1:  # calculate the heading of the actor (approximately)
            ...
            rotate_angles[node_idx] = torch.atan2(
                heading_vector[1], heading_vector[0])
        # only available at one single timestep, use velocity for the heading vector
        elif len(node_historical_steps) == 1:
        ...
        valid[node_idx] = (actor_df['OBJECT_TYPE'].iloc[0] ==
                           'AGENT' and (~padding_mask[node_idx, 25:]).all())
    ...
    node_inds_24 = [actor_identifications.index(list(
        actor_identification)) for actor_identification in df_24[['TRACK_ID', 'VEHICLE']].values.tolist()]
    node_positions_24 = torch.from_numpy(
        np.stack([df_24['X'].values, df_24['Y'].values], axis=-1)).float()
    ...
    output_data = {
        ...
        # [2, num_actor_lane_edges]
        'lane_actor_index': lane_actor_index,
        # [num_actor_lane_edges, 2]
        'lane_actor_vectors': lane_actor_vectors,
        'seq_id': seq_id,  # str
        # [M], for M target agents
        'track_id': torch.tensor(prediction_requests),
        'av_index': av_index,  # int
        # bool [N], True if agent is a vehicle (and not a pedestrian)
        'is_vehicle': is_vehicle,
        # int [M], for M target agents
        'agent_index': torch.tensor(agent_index),
        # bool [M], True if target agent is present in entire future
        'valid': valid[agent_index],
        # bool [Mx#tags], one if agent is performing the maneuver, zero else
        'tags': tags,
        'city': city,  # str
...
def get_lane_features(sm: ShiftsMap,
    ...
    for node_position in node_positions:
        ...
        lane_ids.update(sm.get_lane_ids_in_xy_bbox(
            node_position[0], node_position[1], radius))
    node_positions = torch.matmul(node_positions - origin, rotate_mat).float()
    for lane_id in lane_ids:
        lane_centerline = torch.from_numpy(
            sm.get_lane_centerline(lane_id)[:, :2]).float()
        ...
        availability.append(lane_availability *
                            torch.ones(count, dtype=torch.uint8))
    ...
    lane_actor_index = torch.LongTensor(
        list(product(torch.arange(lane_vectors.size(0)), node_inds))).t().contiguous()
    lane_actor_vectors = \
        lane_positions.repeat_interleave(
            len(node_inds), dim=0) - node_positions.repeat(lane_vectors.size(0), 1)
